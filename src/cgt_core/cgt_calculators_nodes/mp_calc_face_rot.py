...
from .calc_utils import CustomData, ProcessorUtils
...
class FaceRotationCalculator(cgt_nodes.CalculatorNode, ProcessorUtils):
    ...
    def update(self, data, frame=-1):
        ...
        for i in range(4):
            self.data.append([468 + i, [0.0, 0.0, 0.0]])
    ...
    def get_processed_data(self):
        ...
        return (
            ...
            [],
            self.frame,
            self.has_duplicated_results(self.data),
        )
    ...
    def mouth_corners(self):
        ...
        projected_center = cgt_math.project_point_on_vector(
            corner_center, self.data[0][1], self.data[17][1]
        )
    ...
    def set_rotation_driver_data(self):
        ...
        try:
            head_rotation = self.try_get_euler(
                self.pivot.rot, prev_rot_idx=self.pivot.idx
            )
            # head_rotation = self.quart_to_euler_combat(self.pivot.rot, self.pivot.idx, axis='XZY')
        except AttributeError:
            logging.warning(
                "Exchange method in cgt_maths for other targets than blender."
            )
            head_rotation = [0, 0, 0]
    ...
    def chin_rotation(self):
        ...
        self.chin_driver.rot = Euler(((z_angle - 3.14159 * 0.07) * 1.175, 0, 0))
    ...
    def face_mesh_rotation(self):
        ...
        forward_point = cgt_math.center_point(
            np.array(self.data[1][1]), np.array(self.data[4][1])
        )  # nose
        right_point = cgt_math.center_point(
            np.array(self.data[447][1]), np.array(self.data[366][1])
        )  # temple.R
        ...
        try:
            ...
            logging.warning(
                "Exchange method in cgt_math for other targets than Blender."
            )
            quart = None
    ...
    def custom_landmark_origin(self):
        ...
        self.data = [
            [idx, np.array([-lmrk[0], lmrk[2], -lmrk[1]])]
            for idx, lmrk in self.data[:468]
        ]
        self.approximate_pivot_location()
        self.data = [
            [idx, np.array(lmrk) - np.array(self.pivot.loc)]
            for idx, lmrk in self.data[:468]
        ]

    def approximate_pivot_location(self):
        """Sets to approximate origin based on canonical face mesh geometry"""
        right = cgt_math.center_point(
            np.array(self.data[447][1]), np.array(self.data[366][1])
        )  # temple.R
        left = cgt_math.center_point(
            np.array(self.data[137][1]), np.array(self.data[227][1])
        )  # temple.L
