...
class PoseRotationCalculator(cgt_nodes.CalculatorNode, calc_utils.ProcessorUtils):
    ...
    def update(self, data: List, frame: int = -1):
        ...
        for i in range(2):
            self.data.append([33 + i, [0.0, 0.0, 0.0]])
    ...
    def limb_rotations(self):
        """Calculate ik chain rotations using numpy & scipy."""

        def calc_chain_rotations(data, manual_offset=None):
            rotations = []
            for i in range(1, len(data)):
                quart = cgt_math.rotate_towards(data[i][1], data[i - 1][1], "Y", "Z")
                raw_euler = self.try_get_euler(quart, prev_rot_idx=data[i - 1][0])

                if raw_euler is None or len(raw_euler) != 3:
                    continue

                # ป้องกัน raw_euler เป็น mathutils.Euler
                if (
                    hasattr(raw_euler, "x")
                    and hasattr(raw_euler, "y")
                    and hasattr(raw_euler, "z")
                ):
                    raw_euler = [raw_euler.x, raw_euler.y, raw_euler.z]

                # สร้าง rotation จาก raw_euler
                r1 = R.from_euler("xyz", raw_euler)

                if manual_offset is not None and i == len(data) - 1:
                    r_offset = R.from_euler("xyz", manual_offset)
                    r_combined = r1 * r_offset
                    raw_euler = r_combined.as_euler("xyz", degrees=False)
                else:
                    raw_euler = r1.as_euler("xyz", degrees=False)

                rotations.append([data[i - 1][0], raw_euler])
            return rotations

        from math import radians

        hand_rotation_offset1 = [0, 0, 0]
        hand_rotation_offset2 = [0, 0, 0]
        ...
        self.rotation_data += calc_chain_rotations(left_leg)
        self.rotation_data += calc_chain_rotations(right_leg)
        self.rotation_data += calc_chain_rotations(
            left_arm, manual_offset=hand_rotation_offset1
        )
        self.rotation_data += calc_chain_rotations(
            right_arm, manual_offset=hand_rotation_offset2
        )
    ...
    def foot_rotation(self):
        ...
        def rot_from_matrix(loc: List[np.array], tar_idx: int):
            ...
            matrix = cgt_math.generate_matrix(
                *[cgt_math.normalize(vec) for vec in vectors]
            )
    ...
    def torso_rotation(self):
        ...
        hip_center = cgt_math.center_point(
            np.array(self.data[23][1]), np.array(self.data[24][1])
        )
        right_hip = np.array(self.data[24][1])
        shoulder_center = cgt_math.center_point(
            np.array(self.data[11][1]), np.array(self.data[12][1])
        )
        ...
        normal = cgt_math.normal_from_plane(
            [self.data[23][1], self.data[24][1], shoulder_center]
        )
    ...
    def shoulder_rotation(self):
        ...
        shoulder_rotation = cgt_math.rotate_towards(
            shoulder_center, self.data[12][1], "Z"
        )
    ...
    def shoulder_hip_location(self):
        ...
        self.shoulder_center.loc = cgt_math.center_point(
    ...
    def set_hip_as_origin(self):
        ...
        self.data = [
            [
                ...
                np.array(
                    [
                        landmark[0] - self.hip_center.loc[0],
