...
class CGT_PT_MP_Detection(cgt_core_panel.DefaultPanel, bpy.types.Panel):
    ...
    def poll(cls, context):
        return context.mode in {"OBJECT", "POSE"} and all(
            cgt_dependencies.dependencies_installed
        )
    ...
    def movie_panel(self, user):
        ...
        if user.modal_active:
            layout.row().operator(
                "wm.cgt_feature_detection_operator",
                text="Stop Calculate",
                icon="CANCEL",
            )
        else:
    ...
    def json_panel(self, user):
        layout = self.layout
        layout.row().prop(user, "mov_body_data_path")
        layout.row().prop(user, "enum_detection_type")
        if user.modal_active:
            layout.row().operator(
                "wm.cgt_json_feature_detection_operator",
                text="Stop Calculate Json",
                icon="CANCEL",
            )
        else:
            layout.row().operator(
                "wm.cgt_json_feature_detection_operator",
                text="Start Calulate Json",
                icon="IMPORT",
            )

    def text_panel(self, user):
        layout = self.layout
        layout.row().prop(user, "text_input_thai")
        if user.modal_active:
            layout.row().operator(
                "wm.cgt_json_feature_detection_operator",
                text="Stop Calculate Text",
                icon="CANCEL",
            )
        else:
            layout.row().operator(
                "wm.cgt_json_feature_detection_operator",
                text="Start Calulate Text",
                icon="IMPORT",
            )
    ...
    def draw(self, context):
        ...
        elif user.detection_input_type == "json":
            self.json_panel(user)
        elif user.detection_input_type == "text":
            self.text_panel(user)
        else:
...
class CGT_PT_MP_DetectorProperties(cgt_core_panel.DefaultPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if (
            ...
            or user.detection_input_type == "stream"
        ):
...
class CGT_PT_MP_Warning(cgt_core_panel.DefaultPanel, bpy.types.Panel):
    bl_label = "Thai Sign Language Detection"
    bl_parent_id = "UI_PT_CGT_Panel"
    bl_options = {"DEFAULT_CLOSED"}
    ...
    def draw(self, context):
        ...
        lines = [
            f"Please install the missing dependencies for BlendArMocap.",
...
classes = [CGT_PT_MP_Warning, CGT_PT_MP_Detection, CGT_PT_MP_DetectorProperties]
